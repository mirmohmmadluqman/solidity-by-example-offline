---
title: Solidity by Example — Complete Reference
author: solidity-by-example.org  |  Compiled for offline study
date: 2025
---
...
# Solidity by Example
## Complete Reference — 129 Lessons Across 7 Categories

> **How to read this book:** Each lesson shows a title, one-line concept, the full Solidity source, and (for complex topics) bullet-point explanations. Simple lessons are intentionally short.
...
## Chapter 1 — Solidity Basics
  ...
  33. Contract that Creates other Contracts
  34. Import
  35. Library
  36. Hashing with Keccak256
  37. Function Selector
  38. Transient Storage
  39. Assembly Variable
  40. Assembly Conditional Statements
  41. Assembly Loop
  42. Assembly Math
  43. Assembly Error
  44. ABI Encode
  45. ABI Decode
  46. Shadowing Inherited State Variables
  47. Calling Parent Contracts
  48. Try Catch
  49. Unchecked Math
  50. User Defined Value Types
  51. Bitwise Operators
  52. Gas Saving Techniques
...
## Chapter 2 — Applications
  ...
  9. Dutch Auction
  10. Crowd Fund
  11. Precompute Contract Address with Create2
  12. Minimal Proxy Contract
  13. Upgradeable Proxy
  14. Deploy Any Contract
  15. Multi Call
  16. Multi Delegatecall
  17. Time Lock
  18. Uni-Directional Payment Channel
  19. Bi-Directional Payment Channel
  20. Gasless Token Transfer
  21. Merkle Airdrop
  22. Simple Bytecode Contract
  23. Write to Any Slot
  24. Assembly Binary Exponentiation

## Chapter 3 — Hacks & Vulnerabilities
  1. Re-Entrancy
  ...
  12. Source of Randomness
  13. Signature Replay
  14. Bypass Contract Size Check
  15. Deploy Different Contracts at the Same Address
  16. Vault Inflation
  17. WETH Permit
  18. 63 / 64 Gas Rule

## Chapter 4 — EVM Internals
  1. EVM Storage
  2. EVM Memory Layout in Solidity
...
## Chapter 7 — DeFi Protocols
  1. Vault
  2. Staking Rewards
  3. Discrete Staking Rewards
  4. Token Lock
  5. Constant Product AMM
  6. Constant Sum AMM
  7. Stable Swap AMM
  8. Uniswap V2 Swap
  9. Uniswap V2 Add Remove Liquidity
  10. Uniswap V2 Flash Swap
  11. Uniswap V2 Optimal One Sided Supply
  12. Uniswap V3 Swap Examples
  13. Uniswap V3 Liquidity Examples
  14. Uniswap V3 Flash Loan
  15. Uniswap V3 Flash Swap Arbitrage
  16. Chainlink Price Oracle
  17. Chronicle Price Oracle
  18. DAI Proxy Examples
  19. Uniswap V4 Swap
  20. Uniswap V4 Flash Loan
  21. Uniswap V4 Limit Order
...
## 1.5 — Reading and Writing to a State Variable
...
*An example of Ether and Wei in Solidity*
...
*Example of gas and gas limit in Solidity*
...
*If / Else conditional statement in Solidity*
...
## 1.11 — For and While Loop

*Example of for and while loop in Solidity*
...
*Example of using mapping in Solidity*
...
*An example of how to use structs in Solidity*
...
## 1.16 — Data Locations - Storage, Memory and Calldata
...
*Example of how to write functions in Solidity*
...
## 1.18 — View and Pure Functions

*An example of view and pure functions in Solidity*
...
*Example of how to emit events in Solidity*
...
## 1.21 — Events Advanced

*Advanced topics and concepts related to Solidity events*
...
*Example of how to throw errors in Solidity*
...
*An example of interface in Solidity*
...
## 1.28 — Sending Ether (transfer, send, call)

*An example of sending Ether in Solidity*
...
*Example of how to use fallback in Solidity*
...
*Example of how to use delegatecall in Solidity*
...
## 1.33 — Contract that Creates other Contracts
...
*Learn how to import other Solidity files*
...
## 1.36 — Hashing with Keccak256

*Example of hashing using Keccak256 in Solidity*
...
## 1.37 — Function Selector

*Example of how function selectors are computed*
...
## 1.39 — Assembly Variable

*Example of how to declare variable inside assembly*
...
## 1.40 — Assembly Conditional Statements
...
## 1.46 — Shadowing Inherited State Variables
...
## 1.47 — Calling Parent Contracts

*An example of calling parent contracts in Solidity*
...
*An example of try / catch in Solidity*
...
*An example of unchecked math in Solidity*
...
## 1.51 — Bitwise Operators

*Learn about bitwise operators in Solidity*
...
*Simple example of wallet in Solidity*
...
*Example of ERC20 token in Solidity*
...
- ERC-20 is the standard fungible token interface on Ethereum, defined in [EIP-20](https://eips.ethereum.org/EIPS/eip-20).
- `totalSupply` returns the total token supply; `balanceOf` returns a holder's balance.
- `transfer` moves tokens from the caller to a recipient; it MUST emit a `Transfer` event.
- `approve` lets a spender withdraw up to an amount from the caller's account; paired with `transferFrom` for delegated transfers.
- `allowance` returns the remaining amount a spender is permitted to use.
- Key security note: the "approve & transferFrom" pattern has a front-running race condition — use `increaseAllowance`/`decreaseAllowance` or EIP-2612 `permit` to avoid it.
- The `_mint` internal function increases `totalSupply` and credits a balance; `_burn` does the reverse.
- `SafeERC20` (OpenZeppelin) wraps these calls with return-value checks because some tokens (e.g., USDT) do not return `bool`.
...
*Example of ERC721 non-fungible token in Solidity*
...
- ERC-721 defines the non-fungible token (NFT) standard; every token has a unique `tokenId`.
- `ownerOf(tokenId)` returns the owner; `balanceOf(owner)` returns how many NFTs an address holds.
- `transferFrom` and `safeTransferFrom` move ownership; the "safe" variant calls `onERC721Received` on the recipient if it is a contract — prevents tokens being locked in contracts that can't handle NFTs.
- `approve` delegates transfer of a specific tokenId; `setApprovalForAll` delegates the entire collection to an operator (used by marketplaces).
- `tokenURI` should return a JSON metadata URI (often IPFS) describing the NFT.
- Minting is done via an internal `_mint(address, tokenId)` — there is no standard mint function in the spec itself.
- Watch out: `transferFrom` does NOT call `onERC721Received`, so sending to a contract that can't handle NFTs will lock them permanently.
...
*Example of ERC1155 multi token in Solidity*
...
- ERC-1155 is a multi-token standard: one contract can hold multiple token types (fungible, semi-fungible, and non-fungible) identified by `id`.
- `balanceOf(account, id)` and `balanceOfBatch` allow batch queries — more gas-efficient than multiple ERC-20/ERC-721 calls.
- `safeTransferFrom` and `safeBatchTransferFrom` move tokens; the "safe" check calls `onERC1155Received` / `onERC1155BatchReceived` on contracts.
- `setApprovalForAll` delegates the entire collection to an operator.
- URIs use `{id}` as a placeholder replaced client-side with the hex-encoded token id.
- Popular use cases: gaming items, hybrid token sets, gas-optimized airdrops.
...
*Learn about Merkle tree in Solidity*
...
## 2.7 — Multi-Sig Wallet

*An example of multi-sig wallet in Solidity*
...
- A multi-sig wallet requires M-of-N owners to approve a transaction before it executes.
- `submitTransaction` adds a pending tx to the queue; `confirmTransaction` records approvals; `executeTransaction` fires the call once the threshold is met.
- `revokeConfirmation` allows an owner to undo their approval before execution.
- Uses a `mapping(uint => mapping(address => bool))` to track which owners confirmed which tx id.
- `call{value: value}(data)` is used for execution — low-level call forwards gas and allows arbitrary data.
- Key security patterns: check-effects-interactions ordering, reentrancy guard, and requiring the tx has not already been executed.
...
*An example of English auction in Solidity*
...
*An example of Dutch auction in Solidity*
...
*An example of crowdfunding contract*
...
## 2.11 — Precompute Contract Address with Create2
...
- `CREATE2` lets you deploy a contract to a deterministic address computed as: `keccak256(0xff ++ deployer ++ salt ++ keccak256(bytecode))`.
- The address is known before deployment, enabling counterfactual instantiation (e.g., state channels, factory patterns).
- Changing the salt produces a different address from the same bytecode.
- `selfdestruct` followed by re-deploying to the same address with different bytecode is a known attack vector — a contract can "change" its code at a fixed address.
- Used heavily in Uniswap V2 pair factories, Aave, and most major DeFi for deterministic pool addresses.


\newpage

## 2.12 — Minimal Proxy Contract

*Deploy contracts cheaply with minimal proxy contract*
...
- EIP-1167 (minimal proxy / clone factory) deploys ultra-cheap proxy clones that `delegatecall` to a single implementation contract.
- The proxy bytecode is only 45 bytes; deployment cost is ~$0.01 vs hundreds of dollars for a full contract.
- The implementation address is hardcoded into the minimal proxy bytecode at deploy time.
- All calls to the clone are forwarded via `delegatecall` — state is in the clone, logic in the implementation.
- OpenZeppelin's `Clones` library provides `clone(implementation)` and `cloneDeterministic(implementation, salt)`.
- Limitation: the implementation cannot be upgraded after the clone is deployed (no admin slot like EIP-1967).


\newpage

## 2.13 — Upgradeable Proxy

*Example of upgradeable proxy*
...
- The upgradeable proxy pattern separates storage (proxy) from logic (implementation) so logic can be swapped without changing the contract address.
- The proxy uses `delegatecall` to forward all calls to the implementation — the implementation's code runs in the proxy's storage context.
- EIP-1967 defines standard storage slots for the implementation address and admin to avoid slot collisions with implementation variables.
- The "transparent proxy" pattern (used here) restricts the admin from calling implementation functions, preventing selector clashing.
- `_fallback` in the proxy reads the implementation slot and `delegatecall`s to it.
- **Critical gotcha:** never initialize state in an implementation constructor — use an `initialize` function protected by an `Initializable` guard instead.


\newpage

## 2.14 — Deploy Any Contract
...
*An example of contract to aggregate multiple calls.*
...
## 2.18 — Uni-Directional Payment Channel
...
## 2.19 — Bi-Directional Payment Channel
...
- EIP-2612 adds a `permit` function to ERC-20 tokens, allowing approvals via off-chain ECDSA signatures instead of an on-chain `approve` transaction.
- The user signs a structured message (EIP-712 typed data) containing: owner, spender, value, nonce, deadline.
- A relayer submits `permit(owner, spender, value, deadline, v, r, s)` on behalf of the user, paying the gas.
- Nonces prevent replay: each `permit` call increments the owner's nonce, invalidating old signatures.
- Deadline prevents indefinitely-valid signatures — the permit reverts if `block.timestamp > deadline`.
- `GaslessTokenTransfer` combines permit + transferFrom in one atomic tx so the user never pays gas.
...
- Merkle airdrop uses a Merkle tree to commit to a list of eligible recipients off-chain; claimants prove inclusion with a Merkle proof on-chain.
- The root is stored once in the contract; verifying a proof costs O(log N) hashes — far cheaper than storing every eligible address on-chain.
- `MerkleProof.verify(proof, root, leaf)` hashes pairs bottom-up and checks the final hash equals the root.
- Double-claim protection: a `claimed` bitmap or mapping records which leaf indices have been used.
- Security note: leaf hashing must be done carefully — use double-hashing (`keccak256(keccak256(...))`) to prevent second pre-image attacks.


\newpage

## 2.22 — Simple Bytecode Contract
...
## 2.24 — Assembly Binary Exponentiation
...
*An example of re-entrancy attack in Solidity*
...
- Re-entrancy is one of the most famous Solidity vulnerabilities (exploited in the 2016 DAO hack — $60M lost).
- Vulnerable pattern: a contract sends ETH (or makes an external call) BEFORE updating its own state.
- The attacker's `receive()` / `fallback()` calls back into the victim before the first call completes, re-entering with the old (un-decremented) balance.
- **Fix — Checks-Effects-Interactions (CEI):** update all state (balances) BEFORE making external calls.
- **Fix — ReentrancyGuard:** use OpenZeppelin's `nonReentrant` modifier which sets a lock flag.
- Cross-function re-entrancy: the attacker re-enters a *different* function (e.g., calls `withdraw` from inside `deposit`). CEI + a global lock both prevent it.
- Read-only re-entrancy: attacker re-enters to read stale state (view function) used by an oracle. Mitigated by not relying on balances mid-execution.


\newpage

## 3.2 — Arithmetic Overflow and Underflow
...
- Prior to Solidity 0.8.0, integer arithmetic silently wrapped on overflow/underflow (e.g., `uint8(255) + 1 == 0`).
- This was exploited in the BeautyChain (BEC) hack: a multiplication overflow produced a huge token amount from thin air.
- **Solidity ≥ 0.8.0** reverts automatically on overflow/underflow — no `SafeMath` needed for basic operations.
- `unchecked { ... }` blocks opt back into wrapping behavior for gas savings when you've manually verified no overflow can occur.
- Watch for: casting between types (e.g., `uint256 → uint128`) which can truncate without reverting; use explicit checks.
...
- `selfdestruct(recipient)` removes a contract from state and forcefully sends all its ETH to the recipient — even if the recipient has no `receive` function.
- Attack: a contract that assumes `address(this).balance` reflects only ETH sent via its own functions can be broken by forcing ETH into it via `selfdestruct`.
- Example: a game that tracks contributions in a variable but also uses `address(this).balance` for logic — the attacker manipulates the balance cheaply.
- **Fix:** never rely on `address(this).balance` for contract logic; track ETH internally with a `totalDeposited` variable.
- Note: EIP-6780 (Dencun upgrade) restricts `selfdestruct` — it no longer deletes code/storage unless called in the same transaction as deployment.


\newpage

## 3.4 — Accessing Private Data
...
- `private` in Solidity only means other contracts can't call it — it does NOT hide data from off-chain observers.
- Every storage slot is readable with `eth_getStorageAt(contractAddress, slot, blockNumber)`.
- Storage layout is deterministic: slot 0 = first state variable, slot 1 = second, etc. Mappings use `keccak256(key ++ slot)`.
- **Fix:** never store secrets on-chain. For commit-reveal schemes, hash the secret off-chain and reveal it later.
- Sensitive data (private keys, passwords) must NEVER be placed in contract storage or calldata.
...
- `delegatecall` executes the callee's code in the **caller's storage context** — the callee reads/writes the proxy's storage slots.
- Storage collision: if the proxy and implementation have different variable layouts, a `delegatecall` can corrupt unintended slots.
- The Parity Wallet hack (2017, $30M) exploited this: an uninitialized library contract was `delegatecall`ed, and `initWallet` was called as a regular function, taking ownership.
- The second Parity hack: the attacker called `kill()` on the unprotected library, self-destructing it and bricking all wallets that depended on it.
- Safe usage: EIP-1967 standardizes slot positions; OpenZeppelin's `TransparentUpgradeableProxy` avoids collisions via reserved slots.
...
*An example of denial of service hack in Solidity*
...
- DoS via unexpected revert: if a contract iterates over addresses and calls each (e.g., distributing ETH), a malicious contract that reverts on receive can block the entire loop.
- DoS via gas limit: an unbounded array/mapping loop can exceed the block gas limit, making the function permanently uncallable.
- **Fix for revert DoS:** use pull-over-push — instead of pushing ETH to all recipients, let each recipient pull (withdraw) their own funds.
- **Fix for gas DoS:** avoid unbounded loops; paginate; use mappings instead of arrays where possible.


\newpage

## 3.7 — Phishing with tx.origin

*An example of phishing with tx.origin in Solidity*
...
- `tx.origin` is the original EOA that started the transaction chain; `msg.sender` is the immediate caller.
- Phishing attack: a malicious contract tricks a user into calling it; the malicious contract then calls a victim contract that uses `tx.origin` for auth — `tx.origin` is still the user.
- **Fix:** always use `msg.sender` for authentication, never `tx.origin`.
- `tx.origin` is only safe for: checking "was this called directly by an EOA?" — but even that use case is rare and mostly safe to avoid entirely.


\newpage

## 3.8 — Hiding Malicious Code with External Contract
...
*An example of honeypot in Solidity*
...
- A honeypot looks like a vulnerable contract (e.g., appears to have a bug you can exploit) but traps would-be attackers.
- Common trick: the "vulnerability" is gated by a hidden condition in a deployed library or inherited contract not visible in the visible code.
- Example: `HoneyPot` sets a `log` contract address in storage; `Bank` uses it. The attacker sees Bank and tries to exploit re-entrancy, but `log.get()` reverts the attacker's transaction.
- Lesson: always audit ALL contracts a target depends on, including external dependencies deployed at other addresses.
...
- Front-running exploits the public mempool: an attacker sees a pending tx and submits their own with higher gas to be mined first.
- Classic example: DEX price oracle manipulation — seeing a large buy, the attacker sandwiches it (buy before, sell after) for profit.
- Commit-reveal schemes mitigate front-running: users first submit `keccak256(secret + value)`, then reveal the secret in a later block.
- **Minimizing exposure:** use time-weighted average prices (TWAP) rather than spot prices; add slippage tolerance; use private RPC endpoints (Flashbots Protect).
- Auction example in this contract: commit phase hides bids; reveal phase opens them.


\newpage

## 3.11 — Block Timestamp Manipulation
...
- A signature replay attack reuses a valid signature in a context it wasn't intended for.
- Same-chain replay: a `transfer(to, amount, sig)` function signed for one call can be submitted multiple times.
- Cross-chain replay: a signature valid on mainnet is also valid on a fork/testnet with the same `chainId`.
- **Fix:** include a nonce in every signed message and increment it after each use; include `address(this)` and `block.chainid` (EIP-712 does this via `DOMAIN_SEPARATOR`).
- EIP-712 typed structured data hashing provides a standard domain separator covering: contract name, version, chainId, verifying contract address.


\newpage

## 3.14 — Bypass Contract Size Check

*An example of bypassing contract size check*
...
## 3.15 — Deploy Different Contracts at the Same Address
...
## 3.16 — Vault Inflation

*An example of vault inflation*
...
- ERC-4626 vault inflation (donation) attack targets vaults that compute shares from `totalAssets()` which includes the raw token balance.
- Attack sequence: (1) deposit tiny amount to be first LP, getting 1 share; (2) donate large amount directly to vault (bypassing `deposit`), inflating `totalAssets`; (3) next depositor's shares round down to 0 due to integer division.
- Result: the attacker owns all shares and can redeem the victim's deposit too.
- **Fix:** virtual shares/assets — OpenZeppelin's ERC-4626 adds `10 ** decimalsOffset()` virtual shares/assets to the initial calculation so rounding-down attacks are impractical.
- This attack is why ERC-4626 vaults should never launch with zero liquidity.
...
*An example of WETH permit hack*
...
- The WETH9 contract (the canonical Wrapped Ether) does NOT implement EIP-2612 permit, despite some platforms assuming it does.
- Attack: a phishing contract calls `transferFrom(victim, attacker, amount)` after the victim signed a permit — but WETH9's permit does nothing, so transferFrom falls through to a raw allowance check, and the attacker needs a separate approval.
- However, a newer `ERC20Bank` that wraps permit can be exploited if it calls `token.permit(...)` silently — and the token either ignores the call (no-op) or the ERC20Bank doesn't verify the result.
- **Fix:** when building contracts that use permit, verify the allowance AFTER calling permit — don't assume the permit succeeded.
- Always check whether a specific ERC-20 token actually implements permit before relying on it.
...
*Example of EVM storage in Solidity*
...
## 4.2 — EVM Memory Layout in Solidity

*Example of EVM memory in Solidity*
...
*An example of testing contracts with Echidna*
...
## 6.2 — Foundry Authorization

*Foundry authorization example*
...
```solidity
// CONTRACT 'StoreTest' NOT FOUND IN REPO
```
...
- The staking rewards contract distributes a reward token to stakers proportional to their staked amount and time.
- `rewardPerToken` accumulates a global index: `rewardRate * timeDelta / totalSupply` added each second.
- Each user tracks their personal `userRewardPerTokenPaid`; their earned amount is `(rewardPerToken - userRewardPerTokenPaid) * balance`.
- The pattern ensures O(1) per-user accounting regardless of number of stakers — no loops needed.
- `notifyRewardAmount` resets the reward period; if called before the current period ends, remaining rewards are rolled in.
- This pattern is used directly in Synthetix, Uniswap V2 liquidity mining, and hundreds of DeFi projects.


\newpage

## 7.3 — Discrete Staking Rewards
...
- CPAMM implements the core `x * y = k` invariant from scratch.
- `addLiquidity` deposits token0 and token1 in the current ratio; LP shares are minted proportional to the deposit.
- `removeLiquidity` burns LP shares and returns the proportional token amounts.
- `swap` enforces the invariant: after the swap, `(x + dx) * (y - dy) >= x * y` (with fee deducted from `dx`).
- A 0.3% swap fee is implemented by multiplying `amountIn` by 997 and dividing by 1000 before computing the output.
- This is the foundational building block for understanding Uniswap V2, Curve, Balancer, etc.


\newpage

## 7.6 — Constant Sum AMM
...
- StableSwap (Curve's algorithm) uses the invariant: `A*n^n*sum(x_i) + D = A*D*n^n + D^(n+1)/(n^n * prod(x_i))`.
- The amplification coefficient `A` controls the "flatness" of the curve — higher A means prices stay closer to 1:1 for longer, ideal for stablecoins.
- When `A → 0`, it degenerates to Uniswap's constant product. When `A → ∞`, it approaches a constant sum (perfect price stability but no slippage protection).
- D is the total invariant; computing it requires Newton's method iteration (done in `_getD`).
- Used by Curve Finance for USDC/USDT/DAI, stETH/ETH, and other pegged asset pairs.
- Key insight: understanding StableSwap is essential for auditing DeFi protocols handling pegged assets.
...
- Uniswap V2 is an AMM (Automated Market Maker) using the constant product formula: `x * y = k`.
- A liquidity pool holds two tokens; price is determined by the ratio of reserves.
- `swapExactTokensForTokens` and `swapTokensForExactTokens` route through one or more pairs.
- The 0.3% swap fee accrues to liquidity providers, increasing reserves (`k` grows over time).
- Slippage protection: callers set `amountOutMin` / `amountInMax` and a `deadline`.
- Price impact increases with swap size relative to pool reserves — large swaps in shallow pools are expensive.
- Flash swaps allow borrowing tokens within a single transaction, repaying at the end.


\newpage

## 7.9 — Uniswap V2 Add Remove Liquidity
...
## 7.11 — Uniswap V2 Optimal One Sided Supply
...
- Uniswap V3 introduces concentrated liquidity: LPs provide liquidity within a price range `[tickLower, tickUpper]`.
- Capital efficiency is dramatically higher than V2 — LPs can earn more fees with less capital in active ranges.
- Swaps are routed through the `SwapRouter` using `exactInputSingle` (single hop) or `exactInput` (multi-hop with encoded `path`).
- `sqrtPriceX96` encodes the current price as a Q64.96 fixed-point number.
- `fee` tiers: 100 bps (0.01%) for stable pairs, 500 bps (0.05%), 3000 bps (0.3%), 10000 bps (1%).
- Multi-hop path is ABI-encoded as `tokenIn ++ fee ++ tokenMid ++ fee ++ tokenOut`.


\newpage

## 7.13 — Uniswap V3 Liquidity Examples
...
## 7.15 — Uniswap V3 Flash Swap Arbitrage
...
## 7.18 — DAI Proxy Examples

*Borrow and repay DAI using DssProxy*
...
## 7.19 — Uniswap V4 Swap

*Example of swapping tokens on Uniswap V4*
...
```solidity
// CONTRACT 'UniswapV4Swap' NOT FOUND IN REPO
```


\newpage

## 7.20 — Uniswap V4 Flash Loan

*Example of flash loans on Uniswap V4*
...
```solidity
// CONTRACT 'UniswapV4Flash' NOT FOUND IN REPO
```


\newpage

## 7.21 — Uniswap V4 Limit Order

*Example of a limit order hook for Uniswap V4*
...
```solidity
// CONTRACT 'UniswapV4LimitOrder' NOT FOUND IN REPO
```
...
> Source: [solidity-by-example.org](https://solidity-by-example.org) | MIT License
> Compiled for offline study — all code is from the official open-source repository.